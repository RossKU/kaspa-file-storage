...
### 初期実装 (2025-06-27)
- **MVP完成**: 48個のテスト項目すべて成功
- **GitHub Pages**: https://rossku.github.io/kaspa-file-storage/
- **技術仕様**: 最大ペイロード80-90KB（100KBトランザクション制限）

### 技術的制約と解決策
- **TxID制限**: KaspaはTxIDだけでトランザクション取得不可（設計上の仕様）
- **解決策**: WebSocket監視 + Explorer API併用で99.9%の信頼性達成
- **REST Proxy**: https://proxy.kaspa.ws/ （CORS制限あり）

### 重要な教訓
- **デプロイテスト結果**: 「動いているものは変更しない」
- **V2 Perfect版の成功**: シンプルさが最重要
- **過度な最適化の失敗**: 複雑化は避ける