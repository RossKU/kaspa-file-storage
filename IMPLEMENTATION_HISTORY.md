...
#### 主要修正
- **WebSocket切断問題の解決**
  - 1分アイドルタイムアウト後の自動再接続機能追加
  - `startMonitoring`関数に`isConnected()`チェック追加
  - アップロード中のエラー時に自動再接続を試みる

- **コード品質改善**
  - 重複した`startMonitoring`関数を統合（5758行目と6106行目）
  - 5分タイムアウト機能とイベントリスナーを1つの関数に統合
  - エラーハンドリングの強化

#### 技術詳細
- `sendTransaction`関数に「not connected」エラーの検出と再接続処理を追加
- WebSocket監視開始時に接続状態を確認し、必要に応じて再接続
- Kaspaネットワークへの負荷を最小限に維持（必要時のみ接続）

### v4.3.0 - 統一構造とLocalStorage削除 (2025-07-06)
#### 主要変更点
- **.kaspaファイル構造の統一（v3.0）**
  - ファイル/ディレクトリ共通で`encryption`をトップレベルに配置
  - `type`フィールドで'kaspa-file'と'kaspa-directory'を明確に区別
  - 後方互換性を削除（v2.x形式はサポート外）

- **LocalStorageキャッシュの完全削除**
  - 履歴管理をメモリ内のみに変更
  - .kenvファイルが唯一の真実の源
  - フォルダ参照前のキャッシュによる混乱を防止

- **HTMLバージョン管理の一元化**
  - タイトルタグから動的にバージョンを取得する`getAppVersion()`関数追加
  - 今後はタイトルの更新のみでバージョンが反映される

#### 技術詳細
- ダウンロードエラー「Cannot read properties of undefined (reading 'pbkdf2')」を修正
- アップロード時、履歴からの.kaspaファイル作成時の構造を統一
- .kenvファイルのv1.0→v2.0自動移行機能追加
